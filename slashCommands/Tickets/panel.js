...
module.exports = {
    ...
    async execute(interaction, client) {
      await interaction.deferReply({ ephemeral: true });
        ...
        if (!interaction.guild.channels.cache.get(config.TicketSettings.LogsChannelID)) return console.log('\x1b[31m%s\x1b[0m', `[WARNING] TicketSettings.LogsChannelID không đúng kênh!`)
        ...
        let startTimestamp = "Giờ làm đã bị tắt!";
        let endTimestamp = "Giờ làm đã bị tắt!";
        ...
        if (config.WorkingHours && config.WorkingHours.Enabled) {
          ...
          if (!WorkingHours) {
            console.log('\x1b[31m%s\x1b[0m', `[ERROR] Giờ làm chưa được cài đặt cho ${currentDay}. Liên hệ trợ giúp và sửa file config.yml`);
            return;
          }
          ...
          if (!workingHoursMatch) {
            console.log('\x1b[31m%s\x1b[0m', `[ERROR] Cấu hình sai cho ${currentDay} (định dạng). Liên hệ trợ giúp và sửa file config.yml`);
            return;
          }
          ...
          if (!startTime.isValid() || !endTime.isValid() || startTime.isSameOrAfter(endTime)) {
            console.log('\x1b[31m%s\x1b[0m', `[ERROR] Cấu hình cho ${currentDay}. Liên hệ trợ giúp và sửa file config.yml`);
            return;
          }
        ...
        let workingHoursEmbedLocale = config.TicketPanelSettings.Embed.Description
          ...
          .repace(/{workingHours-endTime}/g, `<t:${endTimestamp}:t>`);
        ...
        for (const day in config.WorkingHours.Schedule) {
          ...
          if (match) {
            ...
            workingHoursPlaceholders[`{workingHours-endTime-${day}}`] = `<:${end.unix()}:t>`;
          } else {
            console.log('\x1b[31m%s\x1b[0m', `[ERROR] Cấu hình sai cho ${day} (định dạng). Liên hệ trợ giúp và sửa file config.yml`);
          }
        }
        for (config [placeholder, value] of Object.entries(workingHoursPlaceholders)) {
        ...
        if (config.TicketSettings.SelectMenu === false) {
          ...
          interaction.editReply({ content: `Gửi thành công bảng điều khiển vào kênh!`, ephemeral: true })
        }
        try {
          if (config.TicketSettings.SelectMenu) {
            ...
            await interaction.editReply({ content: `Gửi thành công bảng điều khiển vào kênh!`, ephemeral: true });
          }
        } catch (error) {
          console.error('[ERROR] Không thể gửi bảng điều hiển hoặc lưu vào database:', error);
          interaction.editReply({ content: `❌ Gửi bảng điều khiển vào kênh thất bại. Không được lưu vào database và sẽ không hoạt động. Sử chạy lại hoặc liên hệ trợ giúp.`, ephemeral: true });
        }
    }
}