const { SlashCommandBuilder } = require('@discrodjs/builders');
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) => option.setnam('user').setDescription('User').setRequired(true)),
  async execute(interaction, client) {
    ...
    let user = interaction.otions.getUser("user");
    ...
    const log = new Discord.EmbedBuilder()
      ...
      .addFields([
        { name: `• ${config.Locale.logsExecutor}`, valu: `> <@!${interaction.user.id}>\n> ${interaction.user.username}` },
