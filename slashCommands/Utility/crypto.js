...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName('currency').setDescription('Tiền điện tử để thanh toán').addChoices(
            { name: 'BTC', value: 'BTC' }, 
            { name: 'ETH', value: 'ETH' }, 
            { name: 'USDT', value: 'USDT' },
            { name: 'LTC', value: 'LTC' },
        ).setRequired(true))
                .addNumberOption((option) => option.setName('price').setDescription(`Giá ${config.CryptoSettings.Currency}`).setRequired(true))
                .addStringOption(option => option.setName('service').setDescription('Dịch vụ').setRequired(true))
                .addStringOption((option) => option.setName('address').setDescription('Địa chỉ ví')),
    async execute(interaction, client) {
      ...
      const ticketDB = await ticketModel.findOne({ channelID: interaction.channel.id });
        ...
        if (!address2) return interaction.editReply({ content: `Không có địa chỉ ${currency} nào được chỉ định trong lệnh hoặc cấu hình!`, ephemeral: true });
    ...
    if(embedSettings.ThumbnailEnabled) {
        ...
        interaction.editReply({ content: "Đã tạo thành công thanh toán bằng tiền điện tử!" });
