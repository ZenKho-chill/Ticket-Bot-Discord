    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addNumberOption((option) => option.setName('price').setDescription('Giá').setRequired(true))
                .addStringOption(option => option.setName('service').setDescription('Dịch vụ').setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('stripe')
                .setDescription('Stripe Invoice')
                .addUserOption((option) => option.setName('user').setDescription('Người dùng').setRequired(true))
                .addStringOption(option => option.setName('email').setDescription('Email của khách hàng').setRequired(true))
                .addNumberOption((option) => option.setName('price').setDescription('Giá').setRequired(true))
                .addStringOption(option => option.setName('service').setDescription('Dịch vụ').setRequired(true))),
...
if(subCmd === "paypal") {
        ...
        if(config.PayPalSettings.Enabled === false) return interaction.reply({ content: "Lệnh này đã bị vô hiệu hóa trong cấu hình!", ephemeral: true })
        ...
        client.paypal.invoice.create(invoiceObject, async (err, invoice) => {
          if (err) {
            ...
            if(config.StripeSettings.Enabled === false) return interaction.reply({ content: "Lệnh này đã bị vô hiệu hóa trong cấu hình!", ephemeral: true })
            ...
            client.stripe.customers.create({ email: customerEmail, name: user.username, description: `ID người dùng Discord: ${user.id}` })
            .then((customer) => {
