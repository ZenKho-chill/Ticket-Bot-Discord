...
const ticketPanelModel = require('../models/ticketPanelModel.js');
const reviewsModel = require('../models/reviewsModel.js');
const blacklistModel = require('../models/blacklistedUsersModel.js');
...
for (let i = 1; i <= 8; i++) {
  ...
  if (config[key]?.Questions?.length > 0) {
    ...
    const components = config[key].Questions.Map(question => {
      ...
      input.setLabel(TextInputStyle[question.style.charAt(0).toUpperCase() + question.style.slice(1).toLowerCase()])
...
module.exports = async (client, interaction) =>{
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      if (config.logCommands) console.log(`${color.yello(`[SLASH COMMAND] ${color.cyan(`${interaction.user.username}`)} sử dụng ${color.cyan(`/${interaction.commandName}`)}`)}`);
      return
    } catch (error) {
  ...
  async function handleSelectMenuUpdate(interaction, menuCategory) {
    try {
      ...
      try {
        ...
        console.error('\x1b[31m%s\x1b[0m', `[ERROR] Lỗi khi kết nối bảng điều khiển tin nhắn: ${fetchError.message}`);
        return;
      }
      ...
      try {
        ...
        console.error('\x1b[31m%s\x1b[0m', `[ERROR] Lỗi khi chỉnh sửa bảng điều khiển tin nhắn: ${editError.message}`);
      }
    } catch (error) {
  ...
  async function processTicketCreation(interaction, buttonConfig, customIdentifier, buttonNumber, responses) {
    ...
    if (config.WorkingHours && config.WorkingHours.Enabled && !config.WorkingHours.AllowTicketsOutsideWorkingHours) {
      ...
      if (!workingHours) {
        console.log('\x1b[31m%s\x1b[0m', `[ERROR] Giờ làm việc chưa được cấu hình cho ${currentDay}. Liên hệ trợ giúp và bổ sung file config.yml của bạn.`);
        return;
      }

      const workingHourMatch = workingHours.match(workingHoursRegex);

      if (!workingHourMatch) {
        console.log('\x1b[31m%s\x1b[0m', `[ERROR] Cấu hình sai giờ làm việc cho ${currentDay}. Liên hệ trợ giúp và sửa file config.yml của bạn.`);
        return;
      }
      ...
      const startTime = moment.tz(startDate + ' ' + workingHourMatch[1], 'YYYY-MM-DD H:m', config.WorkingHours.Timezone);
      const endTime = moment.tz(startDate + ' ' + workingHourMatch[2], 'YYYY-MM-DD H:m', config.WorkingHours.Timezone);

      if (!startTime.isValid() || !endTime.isValid() || startTime.isSameOrAfter(endTime)) {
        console.log('\x1b[31m%s\x1b[0m', `[ERROR] Cấu hình sai giờ làm chô ${currentDay}. Liên hệ trợ giúp và sửa file config.yml của bạn.`);
        return;
      }

      const withinWorkingHours = currentTime.isBetween(startTime, endTime);

      // Tạo placeholder giờ làm cho tất cả các ngày
      ...
      for (const day in config.WorkingHours.Schedule) {
        ...
        if (match) {
          const start = moment.tz(startDate + ' ' + match[1], 'YYYY-MM-DD H:m', config.WorkingHours.Timezone);
          const end = moment.tz(startDate + ' ' + match[2], 'YYYY-MM-DD H:m', config.WorkingHours.Timezone);
          workingHoursPlaceholders[`{startTime-${day}}`] = `<t:${start.unix()}:t>`;
          workingHoursPlaceholders[`{endTime-${day}}`] =`<t:${end.unix()}:t>`;
        } else {
          console.log('\x1b[31m%s\x1b[0m', `[ERROR] Cấu hình sai giờ làm cho ngày ${day}. Liên hệ trợ giúp và sửa file config.yml`);
        }
      }

      // Thay thế chỗ giữ chỗ trong chuỗi
      ...
      // Thay thế ngày hôm nay trong giữ chỗ
    ...
    const remainningTimeSeconds = Math.ceil((cooldown + (config.TicketSettings.TicketCooldown * 1000 - Date.now())) / 1000);
    const unixTimestamp = Math.floor(Date.now() / 1000) + remainningTimeSeconds;

    let cooldownEmbedLocale = config.Locale.cooldownEmbedMsg.replace(/{time}/g, `<t:${unixTimestamp}:R>`);
    let cooldownEbed = new EmbedBuilder()
      ...
      .setTimestamp();
    if (Cooldown.has(interaction.user.id)) return interaction.editReply({ embeds: [cooldownEbed], ephemeral: true })

    let logMsg = `\n\n[${new Date().toLocaleString()}] [TICKET CREATON] Nút: ${buttonNumber}, Người dùng: ${interaction.user.username}`;
    fs.appendFile("./logs.txt", logMsg, (e) => {
      if (e) console.log(e);
    });

    // Kiểm tra yêu cầu roles
    ...
    // Kiểm tra nếu role danh sách đen tạo ticket
    ...
    if (!interaction.guild.channels.cache.get(buttonConfig.TicketCategoryID)) return console.log('\x1b[31m%s\x1b[0m', `[WARNING] ${buttonNumber}.TicketCategoryID không đúng dịnh dạng!`)
    ...
    await buttonConfig.SupportRoles.forEach(roleid => {
      ...
      if (!role) return console.log('\x1b[31m%s\x1b[0m', `[WARNING] ${buttonNumber}.SupportRoles không đúng định dạng!`)
      let tempArray = {
        id: role.id,
        allow: ['SendMessages', 'ViewChannel', 'AttachFiles', 'EmbedLinks', 'ReadMessageHistory']
      }
    ...
    const categoryChannels = interaction.guild.channels.cache.filter(channel => channel.parantId === parentCategory.id);
    ...
    if (channelCount >= maxChannelsPerCategory) {
      ...
      const newCategory = await interaction.guild.channels.create({
        ...
        type: 4,
        postition: parentCategory.rawPosition + 1,
    ...
    if (config.PriortySettings.Enabled && config.PriorityRoles.Enabled && priorityRole) {
      priorityLevel = priorityRole.PriorityLevel.toLowerCase();

      const matchingPriorityLvel = config.PrioritySettings.Levels.find(level => level.priority.toLowerCase() === priorityLevel);

      if (matchingPriorityLvel) {
        const { channelName } = matchingPriorityLvel;
        ...
        channel = await interaction.guild.channels.create({
          ...
          topic: config.TicketSettings.ChannelTopic.replace(/{usename}/g, `<@${interaction.user.id}>`).replace(/{category}/g, `${buttonConfig.TicketName}`),
    ...
    channel.send({
      ...
      if (buttonConfig.Questions && buttonConfig.Questions.length > 0) {
        ...
        // Câu hỏi không được bật, tạo model mới mà không có câu hỏi
        const newModel = new ticketModel({
          guildID: interaction.guild.id,
          channelID: channel.id,
          userID: interaction.user.id,
      ...
      // Đặt cooldown khi người dùng tạo ticket
  ...
  async function handleTicketButton(interaction, buttonConfig, buttonNumber) {
    try {
      ...
      if (!buttonConfig) {
        console.error(`Cấu hình sai cho ${configKey}`);
        return interaction.reply({
          content: 'Đã có vấn đề với cấu hình',
          ephemeral: true
        });
      }
      ...
      if (!cachedConfig?.modal) {
        try {
          const customIdentifier = generateUniqueIndentifier();
          ...
          console.error('\x1b[31m%s\x1b[0m', 'Quá trình tạo ticket thất bại:', processError);

          console.log(`- Thời gian lỗi: ${new Date().toISOString()}`);
          console.log(`- Độ trễ gây ra: ${Date.now() - interactionCreationTime} ms`);

          return interaction.reply({
            content: 'Lỗi khi tạo ticket. Vui lòng thử lại',
            ephemeral: true
          });
        }
      }
      ...
      try {
        ...
        console.error('\x1b[31m%s\x1b[0m', 'Lỗi thành phần:', modalError);

        return interaction.reply({
          content: 'Lỗi khi chuẩn bị ticket. Vui lòng liên hệ admin',
          ephemeral: true
        });
      }
      ...
      try {
        ...
        console.error('\x1b[31m%s\x1b', 'Lỗi thành phần:', modalShowError);

        return interaction.reply({
          content: 'Không thể mở form ticket. Vui lòng thử lại',
          ephemeral: true
        });
      }
    } catch (error) {
      console.error('\x1b[31m%s\x1b[0m', 'Lỗi xử lý nút ticket:', error);
      ...
      try {
        if (!interaction.repiled && !interaction.deferred) {
          await interaction.reply({
            content: 'Đã xảy ra lỗi không xác định. Vui lòng thử lại hoặc liên hệ trợ giúp',
            ephemeral: true
          });
  ...
  function generateUniqueIndentifier() {
    ...
    try {
      ...
      if (!buttonConfig || !buttonConfig.Questions) {
        ...
        return interaction.editReply({
          content: 'Ticket cấu hình lỗi. Vui lòng liên hệ hỗ trợ.',
          ephemeral: true
        });
      }
      ...
      try {
        ...
        console.error('Lỗi xử lý phản hồi mô hình:', responseError);
        return interaction.editReply({
      ...
      const [blacklistResult, workingHoursResult] = await Promise.all([
        ...
        console.error('Xác thực lỗi:', validationError);
        return [null, null];
      });
      ...
      try {
        ...
        console.error('Tạo ticket lỗi:', creationError);
        return interaction.editReply({
          content: 'Lỗi khi tạo ticket. Vui lòng thử lại hoặc liên hệ trợ giúp',
          ephemeral: true
        });
      }
    } catch (error) {
      ...
      try {
        if (!interaction.repiled && !interaction.deferred) {
          await interaction.reply({
            content: 'Đã xảy ra lỗi không mong muốn trong quá trình gửi ticket.',
            ephemeral: true
          });
        } else if (interaction.deferred) {
          await interaction.editReply({
  ...
  if (interaction.type === InteractionType.ModalSubmit && interaction.customId.startsWith('questionModal')) {
    ...
    if (!buttonConfig) {
      console.error(`Cấu hình nút không tìm thấy cho: TicketButton${buttonConfigKey}`);
      return;
    }
  ...
  if (interaction.customId === 'closeTicket') {
    ...
    let supportRole = await urlToHttpOptions.checkIfUserHasSupportRoles(interaction)
    ...
    // Đặt closerUserID vào database
    await ticketModel.updateOne({ channelID: interaction.channel.id }, { $set: { closeUserID: interaction.user.id, closeAt: Date.now() } });
  ...
  if (interaction.customId === 'ticketclaim') {
    ...
    if (config.ClaimingSystem.Enabled === false) return interaction.editReply({ content: "Chức năng nhận ticket đã bị tắt trong config!", ephemeral: true })
    ...
    interaction.channel.messages.fetch(ticketDB.msgID).then(async mg => {
  ...
  if (interaction.customId === 'ticketunclaim') {
    ...
    if (config.ClaimingSystem.Enabled === false) return interaction.editReply({ content: "Chức năng nhận ticket đã bị tắt trong config!", ephemeral: true })
    if (ticketDB.claimed === false) return interaction.editReply({ content: "Ticket chưa được nhận!", ephemeral: true })
    ...
    const embed = new EmbedBuilder()
      ...
      .setTimestamp()
      .setFooter({ text: `${config.Locale.ticketUnClaimBy} ${interaction.user.username}`, iconURL: `${interaction.user.displayAvatarURL({ dynamic: true })}` })
  ...
  if (interaction.customId === 'upvote') {
    ...
    if (!suggestion) return interaction.editReply('Không tìm thấy gợi ý trong database.')
    ...
    // Kiểm tra gợi ý đã được chấp nhận hoặc từ chối chưa
    ...
    // Nếu người dùng đã vote, xử lý logic upvote
    ...
    interaction.channel.messages.fetch(suggestion.msgID).then(async msg => {
      ...
      let sugUpvoted = new EmbedBuilder()
        ...
        .setColor("Green")
        .setDescription(upvotedVariable).setFooter({ text: `${interaction.user.username}`, iconURL: `${interaction.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 })}` })
        .setTimestamp();
      ...
      const upvoteLog = new EmbedBuilder()
        .setColor("Green")
        .setDescription(`${config.SuggestionUpvote.ButtonEmoji} | <@!${interaction.user.id}> (${interaction.user.username}) đã **upvote** [bài](https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${suggestion.msgID})!`)
  ...
  if (interaction.customId === 'downvote') {
    ...
    interaction.channel.messages.fetch(suggestion.msgID).then(async msg => {
      ...
      const upvoteLog = new EmbedBuilder()
        .setColor("Red")
        .setDescription(`${config.suggestionDownvote.ButtonEmoji} | <@!${interaction.user.id}> (${interaction.user.username}) đã **downvote** [bài này](https://discord.com/channels/${interaction.guild/id}/${interaction.channel.id}/${suggestion.msgID})!`)
      if (config.SuggestionSettings.LogsChannel && suggestionLogsChannel) suggestionLogsChannel.send({ embeds: [upvoteLog] })
  ...
  if (interaction.customId === 'resetvote') {
    ...
    // Kiểm tra gợi ý đã được chấp nhận hay từ chối chưa
    if (suggestion.status === 'Accepted' || suggestion.status === 'Denied') {
      return interaction.editReply({ embeds: [cantVote], ephemeral: true });
    }
    ...
    interaction.channel.messages.fetch(suggestion.msgID).then(msg => {
      ...
      const upvoteLog = new EmbedBuilder()
        .setColor("Orange")
        .setDescription(`${config.SuggestionResetVote.ButtonEmoji} | <@!${interaction.user.id}> (${interaction.user.username}) đã **reset vote** cho [bài](https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${suggestion.msgID})!`)
      if (config.SuggestionSettings.LogsChannel && suggestionLogsChannel) suggestionLogsChannel.send({ embeds: [upvoteLog] })
  ...
  if (interaction.isMessageContextMenuCommand() && interaction.commandName.startsWith('Accept')) {
    ...
    interaction.channel.messages.fetch(suggestion.msgID).then(msg => {
      ...
      const embed = msg.embeds[0]
      if (config.SuggestionSettings.EnableAcceptDenySystem) embed.fields[1] = { name: `• ${config.Locale.suggestionInformation}`, value: `> **${config.Locale.suggestionFrom}** <@!${suggestion.userID}>\n> **${config.Locale.suggestionUpvotes}** ${suggestion.upVotes}\n> **${config.Locale.suggestionDownvotes}** ${suggestion.downVotes}\n> **${config.Locale.suggestionStatus}** ${config.suggestionStatus.Accepted}` }

      const embedColor = EmbedBuilder.from(embed)
      embedColor.setColor(config.SuggestionStatuesEmbedColors.Accepted)
      ...
      const upvoteLog = new EmbedBuilder()
        .setColor("Green")
        .setDescription(`${config.SuggestionAccept.Emoji} | <@!${interaction.user.username}> (${interaction.user.username}) đã **chấp nhận** [bài](https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${suggestion.msgID})!`)
      if (config.SuggestionSettings.LogsChannel && suggestionLogsChannel) suggestionLogsChannel.send({ embeds: [upvoteLog] })
    });
  }

  // Nút từ chối
  if (interaction.isMessageContextMenuCommand() && interaction.commandName.startsWith('Denu')) {
    if (config.SuggestionSettings.EnableAcceptDenySystem === false) return;
    await interaction.deferReply({ ephemeral: true });

    const suggestion = await suggestion.findOne({ msgID: interaction.targetId });
    if (!suggestion) return interaction.editReply('Không tìm thấy gợi ý trong database')
    ...
    interaction.channel.messages.fetch(suggestion.msgID).then(msg => {
      let deniedVarible = config.Locale.suggestionDenied.replace(/{link}/g, `https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${suggestion.msgID}`);
      let sugAccepted = new EmbedBuilder()
        .setTitle(config.Locale.suggestionDeniedTitle)
        .setColor("Red")
        .setDescription(deniedVarible)
        .setFooter({ text: `${interaction.user.username}`, iconURL: `${interaction.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 })}` })
        .setTimestamp();
      ...
      const embedColor = EmbedBuilder.from(embed)
      embedColor.setColor(config.SuggestionStatuesEmbedColors.Denied)
      ...
      const upvoteLog = new EmbedBuilder()
        .setColor("Red")
        .setDescription(`${config.SuggestionDeny.Emoji} | <@!${interaction.user.id}> (${interaction.user.username}) đã **từ chối** [bài](https://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${suggestion.msgID})!`)
      if (config.SuggestionSettings.LogsChannel && suggestionLogsChannel) suggestionLogsChannel.send({ embeds: [upvoteLog] })
    });
  }

  // Hệ thống đánh giá ticket
  if (interaction.customId === 'ratingSelect') {
    ...
    async function handleStarRating(interaction, rating) {
      ...
      if (!reviewDataDB.alreadyRated || reviewDataDB.alreadyRated === false) {
        // Update reviewModel và đặt rating
        ...
        // Update guildModel với rating và sao totalReview
    ...
    if (interaction.customId === 'modal-whyRating' && reviewDB.reviewDMUserMsgID === interaction.message.id && reviewDB.userID === interaction.user.id) {
      ...
      if (channel) {
        await channel.messages.fetch(reviewDB.reviewDMUserMsgID).then(async (msg) => {
          ...
          // Update reviewModel và set rating
          await reviewsModel.updateOne(
            { reviewDMUserMsgID: interaction.message.id },
            {
          ...
          let ticketCloseLocale = config.TicketUserCloseDM.CloseEmbedMsg.replace(/{guildName}/g, `${guild.name}`).replace(/{closeAt}/g, `<t:${(ticketDB.closeAt / 1000 | 0)}:R>`).replace(/{close-reason}/g, `${ticketDB.closeReason}`);
          ...
          if (!config.TicketUserCloseDM) embed.setDescription(`${ticketCloseRatingLocale}`)
      ...
      const footerText = embedSettings.Footer.text
        .replace('{ticketCreator.username}', ticketAuthor.username)
        .replace('{ticketCatogory}', reviewDB.category)
        .replace('{ticket.totalMessages}', reviewDB.totalMessages)

      // Kiểm tra footer.text không trống trước khi set footer
      if (footerText.trim() !== '') {
      ...
      if (footerText.trim() !== '' && embedSettings.Footer.CustomIconURL !== '' && embedSettings.Footer.IconEnabled) {
        embed.setFooter({
          text: footerText, // Kèm text nếu không trống
  ...
  if (interaction.customId === 'deleteTicket') {
    ...
    // Đặt closerUserID trong database
    await ticketModel.updateOne(
      { channelID: interaction.channel.id },
      {
        $set: {
